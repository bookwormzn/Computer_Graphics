...
window.onload = function init() {
  ...
  // Assuming canvas dimensions are width="400" and height="300"

   // var aspectRatio = 400 / 400;
   // p = perspective(45.0, aspectRatio, 0.1, 100.0);
...
function render() {
   ...
   var eye = vec3(5.0, 5.0, 5.0); // Updated eye position
   var at = vec3(0.0, 0.0, 0.0);
   var up = vec3(0.0, 1.0, 0.0);

   // Set the modelview matrix for the initial view
   var mv = lookAt(eye, at, up); // Set the camera position and orientation
   mv = mult(mv, rotateY(80)); // Rotate 80 degrees around y-axis

   // Draw the axes using the modelview matrix and axes data
   ...
   gl.drawArrays(shapes.axes.type, shapes.axes.start, shapes.axes.size);
    ...
    var left = -4.0, right = 4.0, bottom = -4.0, top = 4.0, near = -10.0, far = 10.0;
    p = ortho(left, right, bottom, top, near, far);
    gl.uniformMatrix4fv(projLoc, gl.FALSE, flatten(transpose(p)));

    p = perspective(45.0, 1.0, 0.1, 100.0);

    // Draw the first wireCube centered at (0,0,0)
    mv = mult(mv, translate(1, 0, 0)); // translate 1 unit in x direction
    gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
    gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);

    // Draw a second cube in blue, rotated 45 degrees around y-axis
    mv = mult(mv, rotateY(45)); // Rotate 45 degrees around y-axis
    mv = mult(mv, translate(0, 1, 0)); // Move to (0,1,0) relative to axes

    // Update the color of the wireCube to blue
    var blueColor = vec4(0.0, 0.0, 1.0, 1.0); // Blue color
    for (var i = shapes.wireCube.start; i < shapes.wireCube.start + shapes.wireCube.size; i++) {
        shapes.wireCube.colors[i] = blueColor;
    }

   // Update the color buffer data with the new cube color
   gl.bufferData(gl.ARRAY_BUFFER, flatten(shapes.wireCube.colors), gl.STATIC_DRAW);

   // Draw the modified wireCube in blue
   gl.uniformMatrix4fv(mvLoc, gl.TRUE, flatten(transpose(mv)));
   gl.drawArrays(shapes.wireCube.type, shapes.wireCube.start, shapes.wireCube.size);
   }
